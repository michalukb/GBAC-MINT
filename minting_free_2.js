...
let wrapper_2_instance = new web3.eth.Contract(wrapper_2_abi, wrapper_2_address);
...
async function mint(){
  ...
  let mintFee = await wrapper_2_instance.methods.mintFee().call()
  ...
  let holding = await wrapper_2_instance.methods.holding(acc[0]).call();
  console.log('display warning: ', holding)
  if (parseInt(holding) > 0 ||  parseInt(count) > 1){

    window.alert("Can only mint one free !")
    return
  }

  wrapper_2_instance.methods.mint(count).send(parameter, (err, transactionHash) => {
